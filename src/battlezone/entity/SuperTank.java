...
public class SuperTank extends Entity {
    public ID id = ID.SuperTank;
    double t = 0;
    Camera camera;
    double speed = 0.06; //ustalamy domyślną prędkość obiektu
    double alfa = 0; // kąt pod jakim poruszał się obiekt
    double beta = alfa; //kąt pod jakim porusza się obiekt
    String classPath = null;

    public SuperTank(Model model, Vector3 position, EntityHandler entityHandler, Camera camera) {
    ...
    public void logic() {
        ...
        dst.y = 0;
        if (dst.magnitude() < 30 && dst.magnitude() >= 7) { // zbliża się do gracza
        ...
        if (dst.magnitude() < 15 && fireTime >= 120) {
            fireTime = 0;
