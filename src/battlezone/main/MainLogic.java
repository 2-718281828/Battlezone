...
public class MainLogic implements Logic {
    ...
    public MainLogic(Camera camera){
        ...
        entityHandler.entities.add(new Enemy1(LoadModel.loadModel(new File(classPath + "/tank.model"), new Color(0,255,0), camera.renderer, camera),new Vector3(0,-7,0), entityHandler));//model, położenie, entityHandler
        entityHandler.entities.add(new Point(LoadModel.loadModel(new File(classPath + "/cube.model"), Color.red , camera.renderer, camera),new Vector3(1,0,0), entityHandler));//model, położenie, entityHandler
